<div class="card-body">
    <div class="form-group">
      ...
      <input type="text" name="name" class="form-control" value="{{ old('name',isset($user)?$user->name:null) }}" id="name" placeholder="Enter user name">
      @error('name') <i class="text-danger">{{ $message }}</i> @enderror
    </div>
    <div class="form-group">
      <label for="name">Email</label>
      <input type="text" name="email" class="form-control" value="{{ old('email',isset($user)?$user->email:null) }}" id="email" placeholder="Enter user email">
  ...
  <div class="form-group">
    ...
    <div class="form-group">
      ...
      <textarea name="address" class="form-control" id="address" cols="30" rows="5">{{ old('address',isset($user)?$user->address:null) }}</textarea>
