            ...
            <div class="row">
              <div class="col-md-12 form-inline">
                <form action="{{ route('admin.order.change.status',[$order->id,\App\Models\Order::STATUS_PROCESSING]) }}" method="post">
                  @csrf
                  @method('PUT')
                  <button class="btn btn-info" onclick="return confirm('Are you confirm to change status?')">Processing</button>
                </form>
                <form action="{{ route('admin.order.change.status',[$order->id,\App\Models\Order::STATUS_SHIPPED]) }}" method="post">
                  @csrf
                  @method('PUT')
                  <button class="btn btn-primary" onclick="return confirm('Are you confirm to change status?')">Shipped</button>
                </form>
                <form action="{{ route('admin.order.change.status',[$order->id,\App\Models\Order::STATUS_DELIVERED]) }}" method="post">
                  @csrf
                  @method('PUT')
                  <button class="btn btn-success" onclick="return confirm('Are you confirm to change status?')">Delivered</button>
                </form>
                <form action="{{ route('admin.order.change.status',[$order->id,\App\Models\Order::STATUS_CANCELED]) }}" method="post">
                  @csrf
                  @method('PUT')
                  <button class="btn btn-danger" onclick="return confirm('Are you confirm to change status?')">Canceled</button>
                </form>
